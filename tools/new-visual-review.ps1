...
function caseHtml(c,index){return '<section class="case" data-index="'+index+'" data-name="'+escapeHtml((c.title||'')+' '+c.id)+'"><h2>'+value(c.title,c.id)+' <span class="count">(3画像)</span></h2><p class="summary">'+value(c.description,'比較画像を確認します。')+'</p><div class="grid">'+
...
'<figure class="card"><div class="imgwrap"><img loading="lazy" src="'+escapeHtml(c.heatmap)+'"></div><figcaption><span class="cname">差がある場所（FLIP '+Number(c.flipMean||0).toFixed(6)+')</span><span class="badge '+(c.passed?'pass':'fail')+'">'+(c.passed?'合格':'不合格')+'</span></figcaption></figure></div>'+
...
document.querySelector('#export').onclick=()=>{states.forEach(s=>s.persist());const review={schemaVersion:1,runId:manifest.runId,reviewedAt:new Date().toISOString(),cases:states.map(s=>({id:s.c.id,decision:s.decision.value||'Needs follow-up',note:s.note.value}))},a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(review,null,2)],{type:'application/json'}));a.download='review.json';a.click();URL.revokeObjectURL(a.href)};
</script></body></html>
'@
