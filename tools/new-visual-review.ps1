...
<div class="toolbar"><button data-mode="side" class="active">左右に並べる</button><button data-mode="blink">交互に表示</button><button data-mode="opacity">重ねて比較</button>
...
const escapeHtml=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])),value=(v,fallback)=>escapeHtml(v==null||v===''?fallback:v),points=Array.isArray(c.reviewPoints)?c.reviewPoints:[],pointList=points.length?'<ul class="review-points">'+points.map(p=>'<li>'+escapeHtml(p)+'</li>').join('')+'</ul>':'<p>見るポイントはまだありません。</p>';
...
'<section class="summary"><h2>'+value(c.title,c.id)+'</h2><p>'+value(c.description,'この比較の説明はまだありません。')+'</p><div class="image-guide"><article><b>'+value(c.referenceLabel,'基準の画像')+'</b><p>'+value(c.referenceDescription,'比較の基準となる画像です。')+'</p></article><article><b>'+value(c.candidateLabel,'確認する画像')+'</b><p>'+value(c.candidateDescription,'変更後の画像です。')+'</p></article></div><h3>見るポイント</h3>'+pointList+'<p class="limitations"><b>この比較の対象外:</b> '+value(c.limitations,'特記事項はありません。')+'</p></section>'+
...
'<div class="visual overlay"><h2>基準画像 / 比較画像</h2><img src="'+escapeHtml(c.reference)+'"><img class="candidate" src="'+escapeHtml(c.candidate)+'"></div><div class="visual heatmap"><h2>差がある場所（FLIP）</h2><img src="'+escapeHtml(c.heatmap)+'"></div></section>'+
