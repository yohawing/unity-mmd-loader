...
namespace Mmd.Tests.Contracts
{
    public sealed class MotionEvaluationOrderContractTests
    {
        ...
        [Test]
        public void BeforePhysicsExcludesDeformAfterPhysicsAppend()
        {
            ...
            Assert.That(beforePhysics.bones.Single(bone => bone.name == "append").localPosition[0],
                Is.EqualTo(0.0f).Within(0.00001f));
            Assert.That(final.bones.Single(bone => bone.name == "append").localPosition[0],
