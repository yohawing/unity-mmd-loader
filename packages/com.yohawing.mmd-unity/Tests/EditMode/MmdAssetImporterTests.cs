...
namespace Yohawing.MmdUnity.Tests
{
    public sealed class MmdAssetImporterTests
    {
        ...
        [Test]
        public void PmxAnimationTabNoticePinsVmdTimelineOwnership()
        {
            string notice = MmdPmxScriptedImporterEditor.VmdNotAutoGeneratedNotice;
        ...
        {
            string pmxPath = Path.Combine(RepositoryRoot, "packages", "com.yohawing.mmd-unity", "Tests", "Fixtures", "Assets", "test_1bone_cube.pmx");
        ...
        {
            string pmxPath = Path.Combine(RepositoryRoot, "packages", "com.yohawing.mmd-unity", "Tests", "Fixtures", "Assets", "test_1bone_cube.pmx");
            string vmdPath = Path.Combine(RepositoryRoot, "packages", "com.yohawing.mmd-unity", "Tests", "Fixtures", "Assets", "test_1bone_cube_motion.vmd");
        ...
        [Test]
        public void ToonShaderSettingsStateUsesCachedAssetDataForBasicCountsAndPreset()
        {
            var pmxAsset = ScriptableObject.CreateInstance<MmdPmxAsset>();
            try
            {
                pmxAsset.Initialize(
                    ...
                    parseSummary: new MmdPmxParseSummary(
                        ...
                        morphCount: 0,
                        materialCount: 4,
                        diffuseTextureReferenceCount: 3,
                        sphereTextureReferenceCount: 1,
                        toonTextureReferenceCount: 2,
                ...
                var state = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                    pmxAsset, MmdPmxShaderPreset.MmdBasicUrpToon);

                Assert.That(state.ShaderPreset, Is.EqualTo("MmdBasicUrpToon"));
                Assert.That(state.MaterialCount, Is.EqualTo(4));
                Assert.That(state.TransparentMaterialCount, Is.EqualTo(1));
                Assert.That(state.DiffuseTextureReferenceCount, Is.EqualTo(3));
                Assert.That(state.SphereTextureReferenceCount, Is.EqualTo(1));
                Assert.That(state.ToonTextureReferenceCount, Is.EqualTo(2));
                Assert.That(state.EdgeMaterialCount, Is.EqualTo(2));
                Assert.That(state.BaseDiffuseTextureReadiness, Does.Contain("3 PMX diffuse texture reference"));
                Assert.That(state.ToonTextureReadiness, Does.Contain("2 PMX toon texture reference"));
                Assert.That(state.SphereTextureHint, Does.Contain("1 PMX sphere texture reference"));
            }
            finally
            {
                Object.DestroyImmediate(pmxAsset);
        ...
        [Test]
        public void ToonShaderSettingsStateReportsOutlinePassReadinessAndNotClaimedWording()
        {
            var pmxAsset = ScriptableObject.CreateInstance<MmdPmxAsset>();
            try
            {
                ...
                Assert.That(state.EdgeMaterialCount, Is.EqualTo(2));
                Assert.That(state.OutlinePassReadiness, Is.EqualTo("MmdBasicUrpToon outline pass"));
                Assert.That(state.OutlineWidthScale, Is.EqualTo("(placeholder)"));
                Assert.That(state.OutlineColorPolicy, Is.EqualTo("(placeholder)"));
                Assert.That(state.ShaderPropertyHandoffDiagnostics, Does.Contain("not claimed"));
                Assert.That(state.ShaderPropertyHandoffDiagnostics, Does.Contain("does not mutate generated materials"));
            }
            finally
            {
                Object.DestroyImmediate(pmxAsset);
        ...
        [Test]
        public void ToonShaderSettingsStateHandlesNoEdgeMaterialsAndNonBasicPreset()
        {
            var pmxAsset = ScriptableObject.CreateInstance<MmdPmxAsset>();
            try
            {
                ...
                var stateBasic = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                    pmxAsset, MmdPmxShaderPreset.MmdBasicUrpToon);
                Assert.That(stateBasic.EdgeMaterialCount, Is.EqualTo(0));
                Assert.That(stateBasic.OutlinePassReadiness, Is.EqualTo("No PMX draw-edge materials"));
                Assert.That(stateBasic.ShadowStepFeather, Is.EqualTo("(placeholder)"));

                // edge==0 path wins regardless of preset
                var stateOther = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                    pmxAsset, (MmdPmxShaderPreset)99);
                Assert.That(stateOther.OutlinePassReadiness, Is.EqualTo("No PMX draw-edge materials"));
            }
            finally
            {
                Object.DestroyImmediate(pmxAsset);
        ...
        [Test]
        public void ToonShaderSettingsStateUsesCurrentPresetForOutlineReadinessWhenEdgeMaterialsExist()
        {
            var pmxAsset = ScriptableObject.CreateInstance<MmdPmxAsset>();
            try
            {
                ...
                var stateBasic = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                    pmxAsset, MmdPmxShaderPreset.MmdBasicUrpToon);
                Assert.That(stateBasic.EdgeMaterialCount, Is.EqualTo(1));
                Assert.That(stateBasic.OutlinePassReadiness, Is.EqualTo("MmdBasicUrpToon outline pass"));

                var stateOther = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                    pmxAsset, (MmdPmxShaderPreset)99);
                Assert.That(stateOther.OutlinePassReadiness, Is.EqualTo("Shader preset summary only"));
            }
            finally
            {
                Object.DestroyImmediate(pmxAsset);
        ...
        [Test]
        public void ToonShaderSettingsStateHandlesNullAssetWithoutParsing()
        {
            // asset==null must not trigger any PMX load; state reports N/A and placeholders
            var state = MmdPmxScriptedImporterEditor.BuildToonShaderSettingsState(
                asset: null, MmdPmxShaderPreset.MmdBasicUrpToon);

            Assert.That(state.ShaderPreset, Is.EqualTo("MmdBasicUrpToon"));
            Assert.That(state.MaterialCount, Is.EqualTo(0));
            Assert.That(state.TransparentMaterialCount, Is.EqualTo(0));
            Assert.That(state.EdgeMaterialCount, Is.EqualTo(0));
            Assert.That(state.BaseDiffuseTextureReadiness, Is.EqualTo("N/A (pending Apply / import)"));
            Assert.That(state.ToonTextureReadiness, Is.EqualTo("N/A (pending Apply / import)"));
            Assert.That(state.OutlinePassReadiness, Is.EqualTo("No PMX draw-edge materials"));
            Assert.That(state.ShadowStepFeather, Is.EqualTo("(placeholder)"));
            Assert.That(state.ShaderPropertyHandoffDiagnostics, Does.Contain("not claimed (skeleton only)"));
            Assert.That(state.UnsupportedPmxMaterialFeatures, Does.Contain("UI skeleton"));
        }

        [Test]
        public void MaterialTexturePersistencePolicyStateReportsNormalPlacementMaterialPersistence()
        {
            // Calls helper directly (IMGUI-independent). Policy is static, no asset load/parse required.
            var state = MmdPmxScriptedImporterEditor.BuildMaterialTexturePersistencePolicyState(asset: null);

            Assert.That(state.ImplementedNow, Is.True);
            Assert.That(state.ImplementedPersistenceFeatureCount, Is.EqualTo(2));
            Assert.That(state.CurrentPolicy, Does.Contain("importer Material sub-assets for normal placement"));
            Assert.That(state.CurrentPolicy, Does.Contain("separate project Texture2D assets"));
            Assert.That(state.CurrentPolicy, Does.Contain("no Texture sub-assets under .pmx"));
            Assert.That(state.CurrentPolicy, Does.Contain("raw-path/runtime fallback"));
            Assert.That(state.SourceReferencePolicy, Does.Contain("string references remain source-of-truth"));
            Assert.That(state.SourceReferencePolicy, Does.Contain("separate Unity Texture2D assets"));
            Assert.That(state.TextureCopyExternalAssetWritePolicy, Does.Contain("explicit opt-in"));
            Assert.That(state.TextureCopyExternalAssetWritePolicy, Does.Contain("no texture copy during normal import"));
            Assert.That(state.NormalScenePlacementMaterialRule, Does.Contain("must use importer Material sub-assets"));
            Assert.That(state.NormalScenePlacementMaterialRule, Does.Contain("material remaps override slots"));
        }

        [Test]
        public void MaterialTexturePersistencePolicyStateHandlesNullAssetWithoutRequiringLoadedAsset()
        {
            // Explicitly verifies null asset path works (policy does not depend on loaded MmdPmxAsset).
            // A non-null asset may be passed but changes nothing for the static policy claims.
            var pmxAsset = ScriptableObject.CreateInstance<MmdPmxAsset>();
            try
            {
                var stateNull = MmdPmxScriptedImporterEditor.BuildMaterialTexturePersistencePolicyState(asset: null);
                var stateWith = MmdPmxScriptedImporterEditor.BuildMaterialTexturePersistencePolicyState(pmxAsset);

                Assert.That(stateNull.ImplementedNow, Is.True);
                Assert.That(stateWith.ImplementedNow, Is.True);
                Assert.That(stateNull.ImplementedPersistenceFeatureCount, Is.EqualTo(2));
                Assert.That(stateWith.ImplementedPersistenceFeatureCount, Is.EqualTo(2));
                Assert.That(stateNull.CurrentPolicy, Does.Contain("importer Material sub-assets"));
                Assert.That(stateNull.CurrentPolicy, Does.Contain("separate project Texture2D assets"));
                Assert.That(stateNull.CurrentPolicy, Does.Contain("no Texture sub-assets under .pmx"));
            }
            finally
            {
                Object.DestroyImmediate(pmxAsset);
